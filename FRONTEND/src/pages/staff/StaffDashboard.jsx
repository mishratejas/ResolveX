...
const StaffDashboard = () => {
  ...
  const fetchInboxData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const activeChats = response.data.data.conversations.filter(c => c.lastMessage !== null);
        setInboxConversations(activeChats);
        setTotalUnread(response.data.data.totalUnread || 0);

        // Map unread counts by ticket ID
        const counts = {};
        response.data.data.conversations.forEach(c => {
          counts[c.complaintId] = c.unreadCount || 0;
        });
  ...
  const handleOpenChat = (complaint) => {
    ...
    const fullComplaint = assignedComplaints.find(c => c._id === complaint._id || c._id === complaint.complaintId) 
                          || { _id: complaint.complaintId || complaint._id, title: complaint.title, status: complaint.status };
    
    setActiveChatComplaint(fullComplaint);
    
    // Optimistically clear the unread badge
    setUnreadCounts(prev => ({ ...prev, [fullComplaint._id]: 0 }));
    setTotalUnread(prev => Math.max(0, prev - (unreadCounts[fullComplaint._id] || 0)));
    
    // Refresh inbox fully after 1 second to sync read status with backend
    setTimeout(fetchInboxData, 1000);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 pb-12">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl p-6 mb-8 text-white shadow-lg flex flex-col md:flex-row md:items-center justify-between">
          ...
          <button
            onClick={() => { fetchAssignedComplaints(); fetchInboxData(); }}
            className="mt-4 md:mt-0 px-4 py-2.5 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-xl font-medium transition-colors flex items-center gap-2 border border-white/20"
          >
            <RefreshCw className="w-5 h-5" /> Refresh Data
          </button>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex bg-gray-100 rounded-lg p-1 overflow-x-auto">
            ...
            >
              ...
              {totalUnread > 0 && (
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs text-white shadow-sm animate-pulse">
                  {totalUnread}
                </span>
              )}
            </button>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
             {inboxConversations.length === 0 ? (
                <div className="p-16 text-center text-gray-400">
                  ...
                  <p className="text-sm mt-1">When an Admin sends you a message, it will appear here.</p>
                </div>
             ) : (
                <div className="divide-y divide-gray-100">
                  {inboxConversations.map(chat => (
                     <button
                       ...
                       className={`w-full p-5 flex items-start gap-4 transition-colors text-left ${unreadCounts[chat.complaintId] > 0 ? "bg-blue-50/50 hover:bg-blue-50" : "hover:bg-gray-50"}`}
                     >
                       <div className="relative">
                         <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 font-bold border border-blue-200">
                            {chat.ticketId ? chat.ticketId.slice(-3) : chat.complaintId.slice(-3).toUpperCase()}
                         </div>
                         {unreadCounts[chat.complaintId] > 0 && (
                            <span className="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-[10px] text-white font-bold border-2 border-white shadow-sm">
                              {unreadCounts[chat.complaintId]}
                            </span>
                         )}
                       </div>
                       <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start mb-1">
                            <h4 className={`font-bold text-sm truncate ${unreadCounts[chat.complaintId] > 0 ? "text-gray-900" : "text-gray-700"}`}>
                              {chat.title}
                            </h4>
                          ...
                          <p className={`text-sm truncate ${unreadCounts[chat.complaintId] > 0 ? "text-gray-900 font-medium" : "text-gray-500"}`}>
                            <span className="font-semibold">{chat.lastMessage.sender}:</span> {chat.lastMessage.text}
                          </p>
                       </div>
                     </button>
                  ))}
                  ...
                  >
                    <div className="flex flex-col lg:flex-row gap-6 justify-between">
                      ...
                      <div className="flex flex-col items-end justify-center gap-3 min-w-[200px] border-t lg:border-t-0 lg:border-l border-gray-100 pt-4 lg:pt-0 lg:pl-6">
                        ...
                        >
                          <MessageSquare className="w-4 h-4" /> Open Chat
                          {unreadCounts[complaint._id] > 0 && (
                            <span className="absolute -top-2 -right-2 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-xs text-white font-bold border-2 border-white shadow-sm animate-bounce">
                              {unreadCounts[complaint._id]}
                            </span>
                          )}
                        </button>
                      </div>
