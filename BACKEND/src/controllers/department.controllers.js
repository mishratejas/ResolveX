...
export const updateDepartment = async (req, res) => {
    try {
        ...
        // Find the department and ensure it belongs to this admin
        const department = await Department.findOne({ _id: id, adminId: currentAdminId });

        if (!department) {
            return res.status(404).json({ success: false, message: 'Department not found' });
        }

        // Extra backend protection: Prevent renaming the default "Other" bucket
        if (department.name.toLowerCase() === 'other') {
            return res.status(403).json({ success: false, message: "The default 'Other' department cannot be modified." });
        }
